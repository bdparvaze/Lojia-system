...
class PosViewModel(application: Application) : AndroidViewModel(application) {
    ...
    var currentTicketName = MutableStateFlow<com.example.util.UiText>(com.example.util.UiText.StringResource(R.string.ticket_1))
    var selectedCustomerName = MutableStateFlow<com.example.util.UiText>(com.example.util.UiText.StringResource(R.string.walk_in_customer))
    ...
    private val _uiToast = MutableSharedFlow<com.example.util.UiText>()
    val uiToast = _uiToast.asSharedFlow()

    init {
        viewModelScope.launch {
            // Seed screenshot items if not already present
    ...
    fun saveCurrentTicket(customName: String? = null) {
        ...
        clearCart()
        currentTicketName.value = com.example.util.UiText.StringResource(R.string.ticket_prefix, _openTickets.value.size + 1)
    }

    fun loadTicket(ticket: OpenTicket) {
        _cartItems.value = ticket.items
        currentTicketName.value = com.example.util.UiText.DynamicString(ticket.name)
        selectedCustomerName.value = com.example.util.UiText.DynamicString(ticket.customerName)
